...
#pragma region PRIVATE_TYPES
		...
		using hierarchy_node_allocator = typename std::allocator_traits<TAllocator>
			::template rebind_alloc<hierarchy_node>;
#pragma endregion
...
#pragma region CREATORS
		...
		{
			m_typeData.emplace(typeid(void), type_data{ mishs::string_id{ 0 }, 0 });
			m_idToTypeIndex.emplace(0, typeid(void));
			m_hierarchy.emplace_back(hierarchy_node{ typeid(void), 0 });
		}
#pragma endregion
...
#pragma region MANIPULATORS
		...
		{
			assert(!is_registered<TType>() && !is_used(a_id));
			m_typeData.emplace(typeid(TType), type_data{ a_id, m_hierarchy.size() });
			m_idToTypeIndex.emplace(a_id, typeid(TType));
			m_hierarchy.emplace_back(typeid(TType), 0);
		}
		...
		void register_type(mishs::string_id const a_id)
		{
			...
			m_hierarchy.emplace_back(typeid(TDerived), m_typeData.find(typeid(TBase))->second.index);
		}
#pragma endregion
...
#pragma region DATA
		...
		std::vector<hierarchy_node, hierarchy_node_allocator> m_hierarchy;
#pragma endregion	
